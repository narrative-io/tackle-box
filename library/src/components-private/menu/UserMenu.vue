      ...
      v-list(nav dense)
        v-list-item(v-for="item in navGroup.items" @click="navItemClicked(item)" :to="item.to")
          ...
          v-list-item-content
            ...
            .status(v-if="item.status && item.status === 'locked'") 
            ...
            .status(v-if="item.status && item.status !== 'locked'" :class="item.status") 
              .status-content-wrapper
                .status-content.text-c-white {{ item.status }}
</template>
...
<style lang="sass" scoped>
  @import "../../styles-private/_user-menu"
</style>